...
int irudiaKargatuhp(POKEMON *pokemon, int Id)
{
    int colorkey;
    SDL_Surface *surface;
    SDL_Renderer *gRenderer = getRenderer();
    ...
    int x = 0;
    char img[100] = {"./img/combat/vida/0.bmp"};
    if (pokemon[Id].hp < pokemon[Id].hp_max / 2)
    {
        strcpy(img, "./img/combat/vida/1.bmp");
    }
    if (pokemon[Id].hp < pokemon[Id].hp_max / 4)
    {
        strcpy(img, "./img/combat/vida/2.bmp");
    }

    if (irudiKop < MAX_IMG)
    {
        surface = SDL_LoadBMP(img);
        int width = surface->w * (pokemon[Id].hp) / pokemon[Id].hp_max;
